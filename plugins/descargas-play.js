...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  if (!text) throw `_*⛄𝐉𝐨𝐭𝐜𝐡𝐮𝐚-𝐛𝐨𝐭-𝐌𝐃♔わ⛄ - PLAY*_\n\n*Hace falta el título o enlace del video de YouTube.*\n\n*Ejemplo:* _${usedPrefix + command} turn it up (feat. 6arelyhuman)_`;
    ...
    throw '_*⛄𝐉𝐨𝐭𝐜𝐡𝐮𝐚-𝐛𝐨𝐭-𝐌𝐃♔わ⛄ - PLAY*_\n\n*Ocurrió un error. Por favor, inténtalo de nuevo más tarde.*';    
    }}
    if (command == 'play2') {
    try {   
    ...
    if (size2 >= limit2) {  
    await conn.sendMessage(m.chat, {text: `_*⛄𝐉𝐨𝐭𝐜𝐡𝐮𝐚-𝐛𝐨𝐭-𝐌𝐃♔わ⛄ - PLAY*_\n\n*Descargue su vídeo en ${video}*`}, {quoted: m});
    return;    
    }     
