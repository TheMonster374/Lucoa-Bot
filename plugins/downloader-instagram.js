...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  if (!args[0]) throw `*ᴇꜱᴄʀɪʙᴇ ʟᴀ ᴜʀʟ ᴅᴇ ᴜɴ ᴠɪᴅᴇᴏ ᴅᴇ ɪɴꜱᴛᴀɢʀᴀᴍ Qᴜᴇ ᴅᴇꜱᴇᴀꜱ ᴅᴇꜱᴄᴀʀɢᴀʀ*.\n\n[🐶] ᴇᴊᴇᴍᴘʟᴏ: ${usedPrefix + command}* https://www.instagram.com/reel/C1tIP73A_U3/?utm_source=ig_web_copy_link`;
  m.reply(global.wait);
  try {
...
for (let i = 0; i < img.length; i++) {
  ...
  try {
    ...
    for (const urRRl of datTa) {
      ...
      const tXXxt = `*ᴜʀʟ:* ${shortUrRRl}`.trim();
      ...
      try {
        ...
        const txt2 = `*ᴜʀʟ:* ${shortUrl2}`.trim();
        await conn.sendFile(m.chat, resultss, 'error.mp4', txt2, m);
      } catch {
        try {
          const resultssss = await instagramdl(args[0]);
          const shortUrl3 = await (await fetch(`https://tinyurl.com/api-create.php?url=${args[0]}`)).text();
          const txt4 = `*ᴜʀʟ:* ${shortUrl3}`.trim();
