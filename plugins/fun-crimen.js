...
if (media === 'text3') return m.reply(`《💰》*${pickRandom(global.robar)}*\n\n${diamond} 💎 𝐃𝐈𝐀𝐌𝐀𝐍𝐓𝐄\n${exp} XP`).catch(global.db.data.users[m.sender].limit += diamond).catch(global.db.data.users[m.sender].exp += money)
if (media === 'text4') return m.reply(`《🚓》${pickRandom(global.robmal)}\n\n${diamond} 💎 𝐃𝐈𝐀𝐌𝐀𝐍𝐓𝐄\n${exp} XP`).catch(global.db.data.users[m.sender].limit -= diamond).catch(global.db.data.users[m.sender].exp -= money) 
}
handler.help = ['robar'];
handler.tags = ['xp'];
handler.command = /^(crime|crimen|Crimen|RobarBanco|Crime)$/i
