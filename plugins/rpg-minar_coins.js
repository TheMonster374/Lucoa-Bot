const handler = async (m, {conn, isPrems}) => { // lastmiming
  ...
  if (new Date - user.lastcoins < 600000) return await conn.reply(m.chat, `*vuelve en ${msToTime(time - new Date())} para continuar minando ${global.rpgshopp.emoticon('money')}⛏️*`, m);
