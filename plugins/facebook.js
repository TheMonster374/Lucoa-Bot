...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  ...
  try {
    ...
    conn.sendFile(m.chat, r2es, 'error.mp4', `*Jotchua - Bot 🐶*`, m);
  } catch (err1) {
    try {
      ...
      try {
        ...
        conn.sendFile(m.chat, VIDEO, 'error.mp4', `*Jotchua - Bot 🐶*`, m);
      } catch (err2) {
        try {
          const ress = await fg.fbdl(args[0]);
          const urll = await ress.data[0].url;
          await conn.sendFile(m.chat, urll, 'error.mp4', '*Jotchua - Bot 🐶*', m);
        } catch (err3) {
          try {
            const res = await fbDownloader(args[0]);
            for (const result of res.download) {
              const ur = result.url;
              await conn.sendFile(m.chat, ur, 'error.mp4', '*Jotchua - Bot 🐶*', m);
            }
          } catch (err4) {
            try {
              ...
              await conn.sendFile(m.chat, url3, 'error.mp4', '*Jotchua - Bot 🐶*', m);
            } catch (err5) {
              try {
                const {result} = await facebookdl(args[0]).catch(async (_) => await facebookdlv2(args[0])).catch(async (_) => await savefrom(args[0]));
                for (const {url, isVideo} of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, '*Jotchua - Bot 🐶*', m);
              } catch (err6) {
                throw `${fg}`;
              }
            }
