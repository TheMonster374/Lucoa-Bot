...
if (!text) {
if (!usuarioExistente) {
fake = { contextInfo: { externalAdReply: { title: `🌟 ¡Colecciona Personajes!`, body: `Compra un personaje y vuelve aquí`, sourceUrl: md, thumbnailUrl: imagen1 }}}
return conn.reply(m.chat, `Use el comando *${usedPrefix}fantasy* o *${usedPrefix}fy* para comprar un personaje`, m, )
}

const fantasyUsuario = usuarioExistente[userId].fantasy
if (fantasyUsuario.length === 0) {
fake = { contextInfo: { externalAdReply: { title: `😅 ¡No tienes Personajes!`, body: `Vuelve a comprar y regresa aquí`, sourceUrl: md, thumbnailUrl: imagen1 }}}
...
fake = { contextInfo: { externalAdReply: { title: `🤨 ¡Verifique el nombre o código!`, body: `Escriba ${usedPrefix + command} para ver sus personajes`, sourceUrl: md, thumbnailUrl: imagen1 }}}
...
fake = { contextInfo: { externalAdReply: { title: `🌟 Personajes de clase: ${imageClass}`, body: `Puedes hacer un solo cambio por 🤩🎟️`, sourceUrl: md, thumbnailUrl: imagen1 }}}
...
fake = { contextInfo: { externalAdReply: { title: `✅ ¡Personaje ${personaje} cambiado!`, body: `🎟️ Tienes Premium por: ${tiempoPremiumFormateado} `, sourceUrl: md, thumbnailUrl: imageURL }}}
...
const tiempoTotalFormateado = formatearTiempo(tiempoTotal * 60 * 1000, true)
fake = { contextInfo: { externalAdReply: { title: `✅ ¡${personajesMismaClase.length} Personajes cambiados!`, body: `🎟️ Tienes Premium por: ${tiempoPremiumFormateado} `, sourceUrl: md, thumbnailUrl: imagen1 }}}
await conn.reply(m.chat, `*Has cambiado a ${personajesMismaClase.length} Personajes por Tiempo premium\n\n🎟️ *Tiempo premium:* \`\`\`${tiempoTotalFormateado}\`\`\``, m,)
let userInDB = fantasyDB.find(userEntry => userEntry[userId])
if (userInDB) {
