...
let handler = async (m, { conn, args, command }) => {
	if (!args[0]) return conn.reply(m.chat, '*Escribe la URL de un video de Facebook que deseas descargar.*', m, adimagen)
	try {
	let config = {
        'id': args[0],
        'locale': 'id'
	...
	await conn.sendMessage(m.chat, { video: { url: HD || SD }, caption: null }, { quoted: estilo})
		...
		conn.reply(m.chat, '*Ocurrió un error inesperado*', m, adimagen)
	}
}
