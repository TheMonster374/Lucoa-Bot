...
let handler = async (m, { conn, text, args, usedPrefix, command }) => {
    ...
    try {
        ...
        if (m.isGroup) {
            conn.sendMessage(m.chat, { video: { url: pp },mimetype: 'image/gif', gifPlayback: true, caption: str.trim(), mentions: [...str.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@s.whatsapp.net') }, { quoted: m });
        } else {
